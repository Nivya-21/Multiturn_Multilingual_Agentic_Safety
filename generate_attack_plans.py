...
def load_and_format_prompts(behavior, set_number, previous_responses=None,
                            attack_language="English", target_functions=None):
...
def process_single_behavior(i, row, agent, temperature, progress_dir, num_sets=5, default_grading_function: str = ""):
    ...
    for set_num in range(1, num_sets + 1):
        ...
        system_prompt, formatted_user_prompt = load_and_format_prompts(
            ...
            attack_language=agent.config.get("attack_language", "English")  # from config
        )
...
def main():
    ...
    args.add_argument(
        ...
        help="Language for conversation snippets in the plan (e.g. 'Hindi', 'French', 'English')."
    )
    args.add_argument(
    ...
    if parsed_args.language:
        ...
        apg_cfg.setdefault("attack_language", "English")
