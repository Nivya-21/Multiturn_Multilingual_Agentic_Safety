            ...
            try:
                ...
                # Get response based on provider
                    ...
                    if any(f"o{i}" in self.model for i in range(1, 6)):
                        ...
                        # Prefer parsed JSON if it's present (JSON mode)
                        msg = resp.choices[0].message
                        if response_format and hasattr(msg, "parsed") and msg.parsed:
                            response = json.dumps(msg.parsed, ensure_ascii=False)
                        elif isinstance(getattr(msg, "content", None), str) and msg.content.strip():
                            response = msg.content
                        else:
                            # Fallback: assemble text blocks if content is list-like (rare)
